...
function showSlide(block, slideIndex = 0, behavior = 'smooth') {
  ...
  block.querySelector('.carousel-banner-slides').scrollTo({ top: 0, left: activeSlide.offsetLeft, behavior });
}
...
function bindEvents(block) {
  ...
  block.querySelectorAll('.carousel-banner-slide').forEach((slide) => slideObserver.observe(slide));
}

function createSlide(row, slideIndex, id) {
  const slide = document.createElement('li');
  slide.dataset.slideIndex = slideIndex;
  slide.setAttribute('id', `carousel-banner-${id}-slide-${slideIndex}`);
...
let carouselBannerId = 0;
export default async function decorate(block) {
  carouselBannerId += 1;
  const id = carouselBannerId;
  block.setAttribute('id', `carousel-banner-${id}`);
